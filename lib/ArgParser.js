...
function addArgIfPossible(result, options, fullArg, argName, argValue)
{
	...
	if(arg === null)
	{
		...
		if(options.parentOptions != null && options.parentResult != null)
		{
			...
			if(addInfo.added)
			{
				if(options.parentResult.onChange)
				{
					options.parentResult.onChange();
				}
			}
...
function addStrayIfPossible(result, options, stray)
{
	...
	{
		for(var i=0; i<options.strayTypes.length; i++)
		{
			var possibleType = options.strayTypes[i];
			if(possibleType instanceof Array)
			{
				for(var j=0; j<possibleType.length; j++)
				{
					var possibleValue = possibleType[j];
					...
					if(valueType == 'string' || valueType == 'boolean' || valueType == 'integer' || valueType == 'float')
					{
						var value = validateValue(valueType, stray);
						if(value === possibleValue)
						{
							result.strays.push(value);
