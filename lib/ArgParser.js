...
function couldBeHttpURL(url)
{
	if(url.split('.').length < 2 || url.indexOf(':/') !== -1 || url.startsWith('/') || url.startsWith('\\'))
	{
		return false;
	}
	return true;
...
function validateValue(type, value)
{
	switch(type)
	{
		...
		case 'urlpattern':
			var urlstr = value.replace("*", "aaa.bbb.ccc");
			try
			{
				new URL(urlstr);
				return value;
