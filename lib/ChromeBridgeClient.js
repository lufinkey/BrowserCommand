...
class ChromeBridgeClient extends EventEmitter
{
	...
	getChromeAPI(completion)
	{
		...
		this.sendRequest('chrome', request, (response, error) => {
			...
			// declare chrome api object
			let chrome = {};

			// create chrome.runtime prototype to handle chrome.runtime.lastError
			class ChromeRuntime
			{
				constructor()
				{
			...
			// create function to execute chrome requests
			...
			chrome = Introspect.create(response, funcExecutor);
			if(!chrome.runtime)
			{
				chrome.runtime = {};
			}
