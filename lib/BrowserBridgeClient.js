...
const { WebExtRuntime } = require('./WebExtAPI');
...
class BrowserBridgeClient extends EventEmitter
{
	...
	connect(completion)
	{
		...
		this._websocket.onerror = (error) => {
			...
			if(!this._socketConnected)
			{
				...
				this._websocket = null;
				if(!this._cancelNextConnectAttempt && this._options.retryConnectTimeout != null && (new Date().getTime()-this._websocketStartTime.getTime()) < this._options.retryConnectTimeout)
				{
	...
	_handleMessage(message)
	{
		...
		switch(message.type)
		{
			...
			case 'serverEvent':
				this.log("ignoring "+message.event+" server event");
				break;
		}
	}
	...
	getAPI(completion)
	{
		...
		this.sendRequest('chrome', request, (response, error) => {
			...
			chrome.runtime = Object.assign(new WebExtRuntime(), chrome.runtime);
