...
class BrowserBridgeClient extends EventEmitter
{
	...
	subscribeToEvent(target, eventPath)
	{
		...
		{
			if(Target.equal(target, cmpSubscription.target) && Introspect.pathsEqual(eventPath, cmpSubscription.eventPath))
			{
	...
	unsubscribeFromEvent(target, eventPath)
	{
		...
		for(var i=0; i<this._eventSubscriptions.length; i++)
		{
			...
			if(Target.equal(target, subscription.target) && Introspect.pathsEqual(eventPath, subscription.eventPath))
			{
	...
	{
		var eventSubscriptions = this._eventSubscriptions.slice(0);
		for(const subscription of eventSubscriptions)
		{
			if(target == null || Target.equal(target, subscription.target))
			{
				this.unsubscribeFromEvent(subscription.target, subscription.eventPath);
			}
		}
	}
