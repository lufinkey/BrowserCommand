...
class BrowserBridgeController extends EventEmitter
{
	...
	_handleServerMessage(message)
	{
		...
		switch(message.type)
		{
			...
			case 'subscribe':
				...
				{
					this.log("unable to subscribe to event: ", message.eventPath);
				}
				break;
			...
			case 'unsubscribe':
				...
				{
					this.log("unable to unsubscribe from event: ", message.eventPath);
				}
				break;
	...
	subscribeToEvent(eventPath)
	{
		...
		{
			if(Introspect.arePathsEqual(eventPath, subscription.eventPath))
			{
				// already subscribed
				return;
			}
	...
	executeRequest(message, completion)
	{
		switch(message.command)
		{
			...
			case 'js.introspect':
				...
				var returnValue = Introspect.query(this._jsExports, message.query);
				var info = Introspect.read(returnValue, WebExtAPI.getIntrospectClasses());
