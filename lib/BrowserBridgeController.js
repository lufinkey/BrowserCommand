	...
	_handleMessage(message)
	{
		...
		switch(message.type)
		{
			case 'request':
				...
				try
				{
					this.executeRequest(message.content).then((response) => {
			...
			switch(message.command)
			{
				case 'js.query':
					...
					if(typeof returnValue == 'function')
					{
						...
						{
							hasCallback = true;
							args[message.callbackIndex] = (result) => {
								// reject if there's a browser runtime error
								if(['chrome','browser'].includes(message.query[0]))
								{
									var lastError = this._jsExports[message.query[0]].runtime.lastError;
									if(lastError)
									{
										reject(lastError);
										return;
					...
					{
						reject(new Error((typeof returnValue)+" is not a function"));
						return;
					}
					// resolve with the return value if there was no callback or promise
				...
				default:
					reject(new Error("invalid command "+message.command));
					break;
			}
		});
	}
