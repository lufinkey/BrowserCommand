...
class ChromeBridgeController extends EventEmitter
{
	...
	_connectToServer()
	{
		...
		websocket.onmessage = (event) => {
			...
			if(message == null)
			{
				this._verboseLog("bad message from server:");
	...
	{
		this._verboseLog("received message fromm server:");
	...
	sendError(socket, responseId, error)
	{
		...
		// send response
		this._verboseLog("sending error response to server:");
	...
	sendResponse(socket, responseId, content)
	{
		...
		// send response
		this._verboseLog("sending response to server:");
		this._verboseLog(response);
