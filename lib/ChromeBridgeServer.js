...
class ChromeBridgeServer extends EventEmitter
{
	...
	_startServer()
	{
		...
		this._server.on('connection', (client, request) => {
			// connection opened
			if(request.headers.origin.startsWith("chrome-extension://") && request.connection.remoteAddress == '127.0.0.1'
				&& request.headers['x-forwarded-for'] == null && request.headers.host == config.HOST+':'+config.PORT)
			{
				...
				client.on('message', (data) => {
					...
					if(message == null)
					{
						this._verboseLog("received bad message from chrome client");
						return;
					}
			...
			else
			{
				...
				client.on('message', (data) => {
					...
					if(message == null)
					{
						this._verboseLog("received bad message from client");
						return;
					}
	...
	_handleClientMessage(client, message)
	{
		...
		this.sendChromeMessage(message, (response, error) => {
			if(error)
			{
	...
	_handleChromeMessage(chromeClient, message)
	{
		...
		for(var i=0; i<this._pendingRequests.length; i++)
		{
			...
			if(request.requestId == message.responseId)
			{
				...
				if(!request.success)
				{
					request.completion(null, new Error(request.error));
					return;
				}
