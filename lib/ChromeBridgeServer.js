...
class ChromeBridgeServer extends EventEmitter
{
			...
			server.on('connection', (client, request) => {
				...
				if(this._isConnectionRequestFromChromeController(request))
				{
					...
					client.on('message', (data) => {
						...
						if(message == null)
						{
							this.log("bad message from chrome client:");
							this.log(data);
							return;
						}
				...
				else
				{
					...
					client.on('message', (data) => {
						...
						if(message == null)
						{
							this.log("bad message from client:");
							this.log(data);
							return;
						}
	...
	sendError(client, responseId, error)
	{
		...
		{
			this.log("ignoring error received for disconnected client:");
			this.log(error);
			return;
		}
		var response = {
	...
	sendResponse(client, responseId, message)
	{
		if(!this._clients.includes(client))
		{
			this.log("ignoring response received for disconnected client:");
			this.log(message);
			return;
		}
		var response = {
