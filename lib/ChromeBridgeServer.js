...
const lockfilePrefix = os.tmpdir()+'/chrome-cmd-';
...
class ChromeBridgeServer extends EventEmitter
{
	...
	listen(completion)
	{
		...
		lockfile.lock(lockfilePrefix+port).then(() => {
			...
			this._server.on('listening', () => {
				...
				this.emit('listening');
				this.emit('listenAttemptFinish', { listening: false, error: null });
			});
	...
	close(completion)
	{
		...
		var wasListening = this._serverListening;
		...
		this._server.close(() => {
			...
			lockfile.unlock(lockfilePrefix+port).then((error) => {
				if(wasListening)
				{
					this.emit('close');
				}
				/*else
				{
					var listenError = new Error("server was manually closed");
					this.emit('failure', listenError);
					this.emit('listenAttemptFinish', { listening: false, error: listenError });
				}*/
				if(completion)
				{
					completion();
