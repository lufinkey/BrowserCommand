...
function WebExtEvent(info)
{
	...
	if(!noInfo && client != null && client.addEventSubscriber && client.removeEventSubscriber
		&& typeof target == 'string' && eventPath instanceof Array)
	{
	...
	this.addListener = (listener) => {
		...
		if(shouldHandleServerEvents && wasEmpty)
		{
			if(connectHandler)
			{
				client._internalEvents.prependListener('connectAttemptFinish', connectHandler);
			}
	...
	this.removeListener = (listener) => {
		...
		if(shouldHandleServerEvents && listeners.length === 0)
		{
			if(connectHandler)
			{
				client._internalEvents.removeListener('connectAttemptFinish', connectHandler);
			}
...
WebExtAPI.create = function(target, client, info, options)
{
	...
	let queryPath = [].concat(options.query);
