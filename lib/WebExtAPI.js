...
WebExtAPI.create = function(target, client, info, options)
{
	...
	const FunctionHandler = (path, ...args) => {
		...
		const promiseExecutor = (resolve, reject) => {
			...
			// send request
			client.sendRequest(target, request).then((response) => {
				// success
				if(useCallback)
				{
					if(callback)
				...
				// error
				setLastError(error, functionPath.join('.'), useCallback);
				if(useCallback)
				{
					if(callback)
					{
		...
		// return promise if using promises, otherwise don't return anything
		if(!useCallback)
		{
