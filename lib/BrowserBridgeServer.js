...
class BrowserBridgeServer extends EventEmitter
{
		...
		switch(message.type)
		{
			case 'request':
				...
				if(matches != null)
				{
					...
					this.sendBrowserMessage(browser, identifier, message.content, (response, error) => {
						if(error)
						{
							this.sendClientError(socket, message.requestId, error);
						}
	...
	sendBrowserMessage(browser, identifier, message, completion)
	{
		if(this._controllers.length == 0)
		{
			completion(null, new Error("browser extension is not connected"));
			return;
		}
		...
		//TODO handle routing
		var browserSocket = this._controllers[0];
