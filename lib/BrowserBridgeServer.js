...
class BrowserBridgeServer extends EventEmitter
{
	...
	verifyControllerIdentifier(identifier)
	{
		if(identifier != null)
		{
			...
			{
				if(controller.browsercmd.identifier === identifier)
				{
					return false;
				}
			}
		...
		if(this._options.allowedIdentifiers && this._options.allowedIdentifiers.length > 0)
		{
			// ensure the identifier is allowed to connect
			for(const cmpId of this._options.allowedIdentifiers)
			{
				if(identifier === cmpId)
				{
					return true;
		...
		if(this._options.blockedIdentifiers && this._options.blockedIdentifiers.length > 0)
		{
			// ensure the identifier is not blocked from connecting
			for(const cmpId of this._options.blockedIdentifiers)
			{
				if(identifier === cmpId)
				{
					return false;
	...
	_handleControllerRegister(socket, info)
	{
		...
		// prevent undefined or empty string from fucking shit up
		if(!info.identifier)
		{
