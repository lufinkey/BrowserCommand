...
class BrowserBridgeServer extends EventEmitter
{
	...
	_handleClientMessage(socket, message)
	{
		...
		switch(message.type)
		{
			case 'request':
				...
				// parse target
				var target = parseTarget(message.target);
				...
				switch(target.type)
				{
					...
					case 'controller':
						// find controller
						var controllerSocket = this.getController(target.name, target.identifier);
			...
			case 'subscribe':
				...
				// parse target
				var target = this.parseTarget(message.target);
				if(target == null)
				{
					this.log("invalid target");
					return;
	...
	_handleControllerRegister(socket, info)
	{
		...
		socket.browsercmd.getTarget = () => {
			return {
				type: socket.browsercmd.type,
				name: socket.browsercmd.browser,
				identifier: socket.browsercmd.identifier
			};
		};
		...
		{
			if(targetsEqual(subscription.target, socket.browsercmd.getTarget()))
			{
	...
	_handleControllerMessage(socket, message)
	{
		...
		switch(message.type)
		{
			...
			case 'event':
				...
				for(const subscription of this._eventSubscriptions)
				{
					if(targetsEqual(subscription.target, socket.browsercmd.getTarget()) && Introspect.arePathsEqual(subscription.eventPath, message.eventPath))
					{
						// send event to subscribed clients
						var sender = stringifyTarget(socket.browsercmd.getTarget());
	...
	getController(browser, identifier)
	{
		for(const controller of this._controllers)
		{
			if(browser === controller.browsercmd.browser)
			{
				if(identifier == null)
				{
					return controller;
				}
				else if(identifier === controller.browsercmd.identifier)
				{
					return controller;
				}
			}
	...
	subscribeClientToEvent(socket, target, eventPath)
	{
		...
		{
			if(targetsEqual(cmpSubscription.target, target) && Introspect.arePathsEqual(eventPath, cmpSubscription.path))
			{
		...
		if(subscription == null)
		{
			...
			switch(target.type)
			{
				...
				case 'controller':
					subscription.receiverSocket = this.getController(target.name, target.identifier);
	...
	unsubscribeClientFromEvents(socket, target = null, eventPath = null)
	{
		...
		for(var i=0; i<this._eventSubscriptions.length; i++)
		{
			...
			if((target == null || targetsEqual(subscription.target, target)) && (eventPath == null || Introspect.arePathsEqual(subscription.eventPath, eventPath)))
			{
	...
	close(completion)
	{
		...
		{
			// if server is starting, wait a frame and try again
			setTimeout(() => {
...
{
	// regex parse target
	var matches = target.match(new RegExp('^(server|controller)(?::([^:\/]+)(?:\/(.*))?)?$'));
	if(matches == null)
	{
		return null;
	}
	...
	target.toString = target.toString.bind(target);
	// validate target
...
function targetsEqual(target1, target2)
{
	if(target1 == null || target2 == null)
	{
		return false;
	}
	if(target1.type !== target2.type || target1.name !== target2.name || target1. target1.identifier !== target2.identifier)
	{
		return false;
	}
	return true;
