...
class BrowserBridgeServer extends EventEmitter
{
			...
			case 'register':
				...
				switch(message.info.type)
				{
					case 'controller':
						...
						socket.browsercmd.browser = message.info.browser;
	...
	_handleClientMessage(socket, message)
	{
		...
		switch(message.type)
		{
			case 'request':
				...
				{
					this.log("missing message recipient");
					this.sendClientError(socket, message.requestId, new Error("missing message recipient"));
				}
				...
				// check for browser:identifier message
				var matches = message.recipient.match(new RegExp('^(chrome)(?::(.*))?$'));
				if(message.recipient)
				{
					// handle browser request request
					this.log("matches ", matches);
					var browser = matches[1];
					var identifier = matches[2];
					// forward message to chrome extension
					this.sendControllerRequest(browser, identifier, message.content, (response, error) => {
						if(error)
						{
							this.sendClientError(socket, message.requestId, error);
						}
	...
	_handleControllerMessage(socket, message)
	{
		...
		{
			case 'response':
				this.log("received response from browser:");
				this.log(message);
	...
	sendControllerRequest(browser, identifier, data, completion)
	{
		if(this._controllers.length == 0)
		{
			completion(null, new Error("browser is not connected"));
			return;
		}
		...
		this.log("sending message to browser:");
		this.log(request);
